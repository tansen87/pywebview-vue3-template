...
PrivilegesRequiredOverrideText1=%1 可以为所有用户(需要管理员权限), 或仅为你安装。
PrivilegesRequiredOverrideText2=%1 可以仅为你, 或为所有用户(需要管理员权限)安装。
...
DownloadingLabel=正在下载额外的文件...
ButtonStopDownload=停止下载(&S)
StopDownload=您确定要停止下载吗？
ErrorDownloadAborted=下载已中止
ErrorDownloadFailed=下载失败: %1 %2
ErrorDownloadSizeFailed=获取文件大小失败: %1 %2
ErrorFileHash1=文件哈希值失败: %1
ErrorFileHash2=无效的文件哈希值: 应为 %1，找到的是 %2
ErrorProgress=无效进度: %1 / %2
ErrorFileSize=无效的文件大小: 应为 %1，找到的是 %2
...
PrepareToInstallNeedsRestart=安装程序需要重启您的电脑。在重启电脑之后，再运行安装程序完成 [name] 的安装。%n%n您想现在重启吗？
...
AbortRetryIgnoreSelectAction=选择动作
...
ExistingFileReadOnly2=现有的文件不能被替换，因为它是只读的。
ExistingFileReadOnlyRetry=去掉只读属性，再试一次(&R)
ExistingFileReadOnlyKeepExisting=保持现有的文件(&K)
ErrorReadingExistingDest=尝试读了现有的文件时发生一个错误:
FileExistsSelectAction=操作选择
FileExists2=文件已经存在。
FileExistsOverwriteExisting=覆盖现有文件(&O)
FileExistsKeepExisting=保留现有文件(&K)
FileExistsOverwriteOrKeepAll=为下一个冲突执行此操作(&D)
ExistingFileNewerSelectAction=操作选择
ExistingFileNewer2=现有文件比尝试安装的安装程序更新。
